...
func main() {
	...
	// Transcripts
	transcripts, _, err := finnhubClient.EarningsCallTranscriptsApi(context.Background()).Id("AAPL_162777").Execute()
	fmt.Printf("%+v\n", transcripts)
	...
	// Social Sentiment
	socialSentiment, _, err := finnhubClient.SocialSentiment(context.Background()).Symbol("AAPL").Execute()
	fmt.Printf("%+v\n", socialSentiment)

	// Invesment theme
	investmentTheme, _, err := finnhubClient.InvestmentThemesThematicInvesting(context.Background()).Theme("financialExchangesData").Execute()
	fmt.Printf("%+v\n", investmentTheme)
